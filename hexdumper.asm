...
.data						; начало сегмента данных
	...
	file db 17				; подготавливаем буфер, первый байт - максимальная длина имени файла
	len db ?				; фактическая длина введеного имени
	buff db 18 dup('$')		; здесь будет храниться имя файла, которое введём
	hex db ?				; здесь будет храниться полученная hex строка
...
readfile:					; читаем первый файл
	...
	mov cx, 19				; будем считывать по 19 байт
...
checkcode:					; проверка не был ли считан весь файл
	...
	jnz readfile			; если есть ещё информация, то переходим к чтению следующей порции данных
